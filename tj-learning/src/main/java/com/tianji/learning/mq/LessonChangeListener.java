...
public class LessonChangeListener {
    ...
    public void listenLessonPay(OrderBasicDTO order){
        ...
        if(order == null || order.getUserId() == null || CollUtils.isEmpty(order.getCourseIds())){
            //CollUtils.isEmpty相当于coll == null || coll.isEmpty()
            // 数据有误，无需处理
            log.error("接收到MQ支付成功消息有误，订单数据为空");
            return;
        }
        // 2.添加课程
    ...
    public void listenLessonRefund(OrderBasicDTO order){
        ...
        if(order == null || order.getUserId() == null || CollUtils.isEmpty(order.getCourseIds())){
            //CollUtils.isEmpty相当于coll == null || coll.isEmpty()
            // 数据有误，无需处理
            log.error("接收到MQ退款消息有误，订单数据为空");
            return;
        }
        // 2.添加课程
        log.debug("监听到用户{}的订单{}，需要删除一个课程{}", order.getUserId(), order.getOrderId(), order.getCourseIds());
        lessonService.deleteLesson(order.getUserId(), order.getCourseIds().get(0));
    }
}